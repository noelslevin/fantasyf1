...
if (isset($_POST['race_entries'])) {
	...
	if (!empty($_POST['driver'])) {
		...
		$query = "UPDATE races SET drivers_entered = 1 WHERE id = '$raceid'";
			$result = mysql_query($query);
			$affected = mysql_affected_rows();
			if ($affected == 1) {
			// One race updated and marked as complete.
				echo "<p>Drivers entered and race marked as complete.</p>";
			}
			else {
			// No race marked complete, or more than 1
			echo "<p>Error. Number of races marked as complete: ".$affected."</p>";
			}
		}
		else {
...
echo "<form action=\"".$_SERVER['PHP_SELF']."?page=fantasyf1_race_entries\" method=\"post\">\n\n";

// Select all drivers
$query = "SELECT driverstoseasons.id, drivers.forename, drivers.surname, teams.team_name FROM drivers, teams, driverstoseasons, teamstoseasons WHERE driverstoseasons.drivers_id = drivers.id AND driverstoseasons.teamstoseasons_id = teamstoseasons.id AND teamstoseasons.teams_id = teams.id AND teamstoseasons.season = '2013' ORDER BY teams.team_name ASC, drivers.forename ASC, drivers.surname ASC";
$result = mysql_query ($query);
...
$query = "SELECT races.id, grandsprix.grand_prix_name, tracks.track_name, races.race_date FROM grandsprix, races, tracks, trackstograndsprix WHERE races.trackstograndsprix_id = trackstograndsprix.id AND trackstograndsprix.tracks_id = tracks.id AND trackstograndsprix.grandsprix_id = grandsprix.id AND races.drivers_entered = '0' ORDER BY races.race_date ASC";
$result = mysql_query ($query);
if (mysql_num_rows ($result) > 0) {
