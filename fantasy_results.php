...
$query = "UPDATE races SET complete = '1' WHERE races.id = '$raceid'";
$result = mysql_query ($query);
...
$query = "SELECT races.id, grandsprix.grand_prix_name, tracks.track_name, races.race_date FROM grandsprix, races, tracks, trackstograndsprix WHERE races.trackstograndsprix_id = trackstograndsprix.id AND trackstograndsprix.tracks_id = tracks.id AND trackstograndsprix.grandsprix_id = grandsprix.id AND races.complete = '1' ORDER BY races.race_date DESC";
$result = mysql_query ($query);
if (mysql_num_rows ($result) > 0) {
	echo "<form action=\"".$_SERVER['PHP_SELF']."?page=fantasyf1_fantasy_results\" method=\"post\">\n";
	echo "<select name = \"race_id\">\n";
