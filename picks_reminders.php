...
$query = "SELECT fantasyuserstoseasons.id, fantasyusers.username, fantasyteamstoseasons.teamname, fantasyusers.email_address FROM fantasyusers INNER JOIN fantasyteams INNER JOIN fantasyuserstoseasons ON (fantasyuserstoseasons.fantasyusers_id = fantasyusers.id) INNER JOIN fantasyteamstoseasons ON (fantasyteamstoseasons.fantasyteams_id = fantasyteams.id AND fantasyteamstoseasons.id = fantasyuserstoseasons.fantasyteamstoseasons_id) WHERE fantasyteamstoseasons.season = 2013 ORDER by fantasyusers.username ASC";
$result = mysql_query($query);
$num = mysql_num_rows ($result);
if ($num > 0) {
	echo "<table>\n
	<thead>\n
	<tr><th>Username</th><th>Team</th><th>Email Address</th></tr>\n
	</thead>\n
	<tbody>\n";
